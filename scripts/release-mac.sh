...
CONFIG_PUBLIC_KEY="$(node -e 'const fs=require("node:fs"); const conf=JSON.parse(fs.readFileSync("src-tauri/tauri.conf.json","utf8")); process.stdout.write(conf?.plugins?.updater?.pubkey ?? "");')"
KEYCHAIN_PUBLIC_KEY_B64="$(printf "%s" "$PUBLIC_KEY" | base64 | tr -d '\n')"
...
if [ "$CONFIG_PUBLIC_KEY" != "$KEYCHAIN_PUBLIC_KEY_B64" ]; then
